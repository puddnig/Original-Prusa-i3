...
module bolthole(){
 ...
 module zab_left(){
    translate([0,0,xstepper_width/2+2*zab_reinf_outer,0]) 
     difference(){
    translate([-zrod_frame_distance,-Z_axis_offset-xstepper_width/2-zab_reinf_outer,0])cube([zrod_frame_distance+xstepper_width/2,xstepper_width+zab_reinf_inner+zab_reinf_outer+Z_axis_offset,zab_height]); //outer cube
    difference(){
        translate([-zrod_frame_distance+zab_frame_flange,-Z_axis_offset-xstepper_width/2,zab_stepper_flange])
            cube([zrod_frame_distance+xstepper_width/2,xstepper_width+Z_axis_offset,zab_lower_bolt+zab_bolt_spacing+zab_frame_flange]);
            
            translate([-zrod_frame_distance+zab_frame_flange,-xstepper_width/2-Z_axis_offset,zab_stepper_flange])rotate([0,90,0])chamfer(2,200);
    ...
    translate([0,-Z_axis_offset,0]){
    translate([0,-zab_bolt_spacing/2,zab_lower_bolt]) bolthole();
    translate([0,zab_bolt_spacing/2,zab_lower_bolt]) bolthole();
    translate([0,0,zab_lower_bolt+zab_bolt_spacing]) bolthole();}
    translate([-zrod_frame_distance+zab_frame_flange,0,zab_height-4])cube([2*(zab_frame_flange-zab_bolt_clamping_length),2.5*m3_through_dia,2*(zab_height-zab_lower_bolt-zab_bolt_spacing)],center=true);
    
    translate([xstepper_width/2,xstepper_width/2+zab_reinf_inner,0])rotate([0,0,0])fillet(8,30);
translate([xstepper_width/2,-xstepper_width/2-zab_reinf_outer-Z_axis_offset,0])rotate([0,0,270])fillet(8,30);
    
    translate([-zrod_frame_distance,-zab_reinf_outer-xstepper_width/2-Z_axis_offset,zab_height])rotate([180,-90,0])fillet(10,20);
translate([-zrod_frame_distance,zab_reinf_inner+xstepper_width/2,zab_height])rotate([0,-90,0])fillet(5,20);
    
   
translate([-zrod_frame_distance,-xstepper_width/2-zab_reinf_outer+5-Z_axis_offset,0]) corner();
