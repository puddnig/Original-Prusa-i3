...
translate([-dout+din/2,-dout/2,dep])cube([dout-din,dout,2.1*lh]);
}
module corner(){
...
module quad_hole_bottom(din,dout,dep,lh){
module layer(){
translate([din/2,-dout/2-0.5,dep])cube([dout-din,dout+1,2.1*lh]);
translate([-dout+din/2,-dout/2-0.5,dep])cube([dout-din,dout+1,2.1*lh]);
}
difference(){
translate([-dout/2,-dout/2,0])cube([dout,dout,dep+2*lh]);
layer();
rotate([0,0,90])translate([0,0,lh])layer();
}

}
quad_hole_bottom(3.2,6,0,0.2);
